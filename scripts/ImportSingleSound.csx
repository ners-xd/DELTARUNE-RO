...
{
    case "DELTARUNE":
        soundPath = Path.Combine(Path.GetDirectoryName(ScriptPath), "../mod/chapter_select/sounds");
        break;

    case "DELTARUNE Chapter 1":
        soundPath = Path.Combine(Path.GetDirectoryName(ScriptPath), "../mod/chapter1/sounds");
        break;

    case "DELTARUNE Chapter 2":
        soundPath = Path.Combine(Path.GetDirectoryName(ScriptPath), "../mod/chapter2/sounds");
        break;

    case "DELTARUNE Chapter 3":
        soundPath = Path.Combine(Path.GetDirectoryName(ScriptPath), "../mod/chapter3/sounds");
        break;

    case "DELTARUNE Chapter 4":
        soundPath = Path.Combine(Path.GetDirectoryName(ScriptPath), "../mod/chapter4/sounds");
        break;

    case "DELTARUNE Chapter 5":
        soundPath = Path.Combine(Path.GetDirectoryName(ScriptPath), "../mod/chapter5/sounds");
        break;

    case "DELTARUNE Chapter 6":
        soundPath = Path.Combine(Path.GetDirectoryName(ScriptPath), "../mod/chapter6/sounds");
        break;

    case "DELTARUNE Chapter 7":
        soundPath = Path.Combine(Path.GetDirectoryName(ScriptPath), "../mod/chapter7/sounds");
        break;
}

// Determine basic sound name properties.
foreach(DirectoryInfo dir in new DirectoryInfo(soundPath).GetDirectories())
{
    foreach(FileInfo file in dir.GetFiles())
    {
        ...
        for (int i = 0; i < Data.Sounds.Count; i++)
        {
            if (Data.Sounds[i]?.Name?.Content == soundName)
            {
                ...
                replaceSoundPropertiesCheck = true;
                break;
            }
        }
        ...
        {
            soundData = new UndertaleEmbeddedAudio() { Data = File.ReadAllBytes(file.FullName) };
